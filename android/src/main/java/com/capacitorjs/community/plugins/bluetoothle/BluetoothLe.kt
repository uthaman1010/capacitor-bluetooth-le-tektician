...
class BluetoothLe : Plugin() {
  companion object {
    ...
    private const val MAX_SCAN_DURATION: Long = 10000
  ...
  fun requestLEScan(call: PluginCall) {
    ...
    deviceScanner?.startScanning(scanFilters,
      ...
      { result ->
        run {
          val scanConcluded = JSObject()
          scanConcluded.put("message", result.message)
          scanConcluded.put("success", result.success)
          try {
              notifyListeners("onScanComplete", scanConcluded)
          }catch (e: ConcurrentModificationException){
            Logger.error(TAG, "Error in notifyListeners: ${e.localizedMessage}", e)
          }
        }
