...
class DeviceScanner(
  ...
  private var stopCallBack: ((ScanConcluded) -> Unit)? = null
  ...
  fun startScanning(
    ...
    stopCallBack: ((ScanConcluded) -> Unit)?
  ) {
  ...
  fun stopScanning(postDelayed: Boolean = false) {
    ...
    isScanning = false
    stopCallBack?.invoke(
      ScanConcluded(postDelayed, "Stopped scanning.")
    )
